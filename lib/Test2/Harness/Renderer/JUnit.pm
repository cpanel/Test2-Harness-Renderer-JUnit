...
sub render_event {
    ...
    if ( $f->{'harness_job_end'} ) {
        ...
        push @{ $test->{'testcase'} }, $self->xml->testcase( { 'name' => "Tear down.", 'classname' => $test->{'file'}, 'time' => $stamp - $test->{'last_job_start'} }, "" );
    ...
    if ( $f->{'assert'} ) {
        ...
        if ( $f->{'amnesty'} && grep { ( $_->{'tag'} // '' ) eq 'TODO' } @{ $f->{'amnesty'} } ) {    # All TODO Tests
            if ( !$f->{'assert'}->{'pass'} ) {                                                       # Failing TODO
                push @{ $test->{'testcase'} }, $self->xml->testcase( { 'name' => "$test_string (TODO)", 'classname' => $test->{'file'}, 'time' => $run_time }, "" );
            }
                ...
                push @{ $test->{'testcase'} }, $self->xml->testcase( { 'name' => "$test_string (PASSING TODO)", 'classname' => $test->{'file'}, 'time' => $run_time }, "" );
            }
                ...
                push @{ $test->{'testcase'} }, $self->xml->testcase(
                    { 'name' => "$test_string (TODO)", 'classname' => $test->{'file'}, 'time' => $run_time },
                    $self->xml->error(
            ...
            push @{ $test->{'testcase'} }, $self->xml->testcase( { 'name' => "$test_string", 'classname' => $test->{'file'}, 'time' => $run_time }, "" );
        }
        else {                                  # Failing Test.
            $test->{'testsuite'}->{'failures'}++;
            $test->{'testsuite'}->{'errors'}++;
...
sub close_open_failure_testcase {
    ...
    push @{ $test->{'testcase'} }, $xml->testcase(
        { 'name' => "$fail->{test_num} - $fail->{test_name}", 'classname' => $test->{'file'}, 'time' => $fail->{'time'} },
        $xml->failure(
